...
- visit_date (необязательно): Дата посещения медицинского учреждения пациентом (формат YYYY-MM-DD)
...
- visit_date (необязательно): Дата посещения медицинского учреждения или лаборатории пациентом (формат YYYY-MM-DD)
- report_date (обязательно): Дата создания отчета (формат YYYY-MM-DD)
- collection_date (необязательно): Дата сбора образцов или дата регистрации материала (формат YYYY-MM-DD). ВНИМАНИЕ: Ищите эту дату в документе под различными названиями, такими как "Дата регистрации", "Дата сбора", "Дата взятия материала", "Дата забора", "Дата/время регистрации". Извлекайте только дату, игнорируйте время.
...
- patient_age (необязательно): Возраст пациента
- patient_contact (необязательно): Контактный номер или email пациента
- visit_date (обязательно): Дата посещения медицинского учреждения пациентом (формат YYYY-MM-DD)
